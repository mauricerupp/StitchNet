...
def create_training_data(raw_dir, target_dir, snap_dir, paths_dir, target_size, snap_size, snaps_per_sample, step_size):
    ...
    for img in os.listdir(raw_dir):
        ...
        space = (snaps_per_sample - 1)*step_size + snap_size[0] + 1
        if h < space:
        ...
        img_snaps, covered_pixels, img_overlapse, middle_frame_center = create_fix_translation_path(img_target,
                                                                                                    snaps_per_sample,
                                                                                                    snap_size,
                                                                                                    step_size)
        sample_count += 1
...
def create_fix_translation_path(img_target, snaps_per_sample, snap_size, step_size):
    ...
    for iterationCount in range(snaps_per_sample):
        ...
        if iterationCount == 0:
            ...
            top_left_corner = update_frame_position(top_left_corner, step_size, h_snap, h_target, w_snap, w_target)
