...
def create_training_data(raw_dir, target_dir, snap_dir, paths_dir, target_size, snap_size, snaps_per_sample):
    ...
    for img in os.listdir(raw_dir):
        ...
        img_snaps, covered_pixels, img_overlapse, middle_frame_center, x1, y1 = create_linear_translation_path(img_target, snaps_per_sample, snap_size)
        img_snaps, covered_pixels, img_overlapse, middle_frame_center, x2, y2 = create_linear_translation_path(
            img_target, snaps_per_sample, snap_size)
        img_snaps, covered_pixels, img_overlapse, middle_frame_center, x3, y3 = create_linear_translation_path(
            img_target, snaps_per_sample, snap_size)
        img_snaps, covered_pixels, img_overlapse, middle_frame_center, x4, y4 = create_linear_translation_path(
            img_target, snaps_per_sample, snap_size)
        img_snaps, covered_pixels, img_overlapse, middle_frame_center, x5, y5 = create_linear_translation_path(
            img_target, snaps_per_sample, snap_size)
        img_snaps, covered_pixels, img_overlapse, middle_frame_center, x6, y6 = create_linear_translation_path(
            img_target, snaps_per_sample, snap_size)
        img_snaps, covered_pixels, img_overlapse, middle_frame_center, x7, y7 = create_linear_translation_path(
            img_target, snaps_per_sample, snap_size)
        img_snaps, covered_pixels, img_overlapse, middle_frame_center, x8, y8 = create_linear_translation_path(
            img_target, snaps_per_sample, snap_size)
        img_snaps, covered_pixels, img_overlapse, middle_frame_center, x9, y9 = create_linear_translation_path(
            img_target, snaps_per_sample, snap_size)
        img_snaps, covered_pixels, img_overlapse, middle_frame_center, x10, y10 = create_linear_translation_path(
            img_target, snaps_per_sample, snap_size)
        img_snaps, covered_pixels, img_overlapse, middle_frame_center, x11, y11 = create_linear_translation_path(
            img_target, snaps_per_sample, snap_size)
        img_snaps, covered_pixels, img_overlapse, middle_frame_center, x12, y12 = create_linear_translation_path(
            img_target, snaps_per_sample, snap_size)
        img_snaps, covered_pixels, img_overlapse, middle_frame_center, x13, y13 = create_linear_translation_path(
            img_target, snaps_per_sample, snap_size)
        img_snaps, covered_pixels, img_overlapse, middle_frame_center, x14, y14 = create_linear_translation_path(
            img_target, snaps_per_sample, snap_size)
        img_snaps, covered_pixels, img_overlapse, middle_frame_center, x15, y15 = create_linear_translation_path(
            img_target, snaps_per_sample, snap_size)
        ax = plt.gca()
        ax.set_xlim([0, h])
        ax.set_ylim([0, w])
        plt.plot(x1,y1,x2,y2,x3,y3,x4,x5,y5,x6,y6,x7,y7,x8,y8,x9,y9,x10,y10,x11,y11,x12,y12,x13,y13,x14,y14,x15,y15, marker="", linestyle='-')
        plt.savefig("/home/maurice/Dokumente/BA/Docu_linear_trainingdata/paths{}.png".format(sample_count))
        plt.close()
...
def create_linear_translation_path(img_target, snaps_per_sample, snap_size):
    ...
    top_left_corners_list.append(top_left_corner.copy())
    # get the first snap
    ...
    top_left_corners_list = np.array(top_left_corners_list)
    x,y = np.split(top_left_corners_list, 2, axis=1)
    print(x)
    x += int(snap_size[0]/2)
    y += int(snap_size[1]/2)

    assert img_snaps.shape == (h_snap, w_snap, 3 * snaps_per_sample) #since there are 3 channels per snap
    return img_snaps, covered_area, img_overlapse, middle_frame_center, x, y
